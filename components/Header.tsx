...
export function Header() {
  ...
  return (
    <header className="relative z-50 bg-black">
      <div className="container-wide flex h-24 items-center justify-between gap-6">
        ...
        <nav className="hidden items-center gap-10 lg:flex" aria-label="Hoofdmenu">
          {nav.map((item) => {
            const active =
              item.href === "/"
                ? pathname === "/"
      ...
      <div className={`lg:hidden ${open ? "pointer-events-auto" : "pointer-events-none"}`}>
        <div
          className={`absolute inset-x-0 top-24 z-40 origin-top transition-all duration-200 ${
